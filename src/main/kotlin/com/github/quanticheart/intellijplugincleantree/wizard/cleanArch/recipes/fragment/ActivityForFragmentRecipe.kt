package com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.recipes.fragment

import com.android.tools.idea.wizard.template.ModuleTemplateData
import com.android.tools.idea.wizard.template.RecipeExecutor
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.res.layout.activityForFragmentXml
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.res.layout.mvvmXml
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.ui.cleanArchActivity
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.ui.cleanArchActivityForActivity
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.ui.cleanArchFragment
import com.github.quanticheart.intellijplugincleantree.wizard.cleanArch.ui.cleanArchViewModelSimple
import java.util.*

fun RecipeExecutor.activityAndVMRecipe(
    moduleData: ModuleTemplateData,
    featureName: String,
    layoutName: String,
    packageName: String
) {

    val (projectData, srcOut, resOut) = moduleData

    /**
     * Packages
     */
    val applicationPackage = projectData.applicationPackage
    val featurePackage = "${packageName}.${featureName.toLowerCase()}"

    //ui
    val activityName = "${featureName}Activity"
    val fragmentName = "${featureName}Fragment"
    val activityViewBindingName = "Activity${featureName}Binding"
    val fragmentViewBindingName = "Fragment${featureName}Binding"
    val viewModelName = "${featureName}ViewModel"

    val xmlActivityName = layoutName.replace("fragment", "activity")

    /**
     * Paths
     */
    val basePath = featureName.toLowerCase(Locale.getDefault())

    /**
     * Create Files
     *
     * Layout
     */
    val xml = activityForFragmentXml()
    save(xml, resOut.resolve("layout/$xmlActivityName.xml"))

    val xmlFragment = mvvmXml()
    save(xmlFragment, resOut.resolve("layout/$layoutName.xml"))

    /**
     * UI
     */
    val activity = cleanArchActivityForActivity(
        applicationPackage = applicationPackage,
        featurePackage = featurePackage,
        activityName = activityName,
        viewBindingName = activityViewBindingName,
        fragmentName = fragmentName
    )
    save(activity, srcOut.resolve("$basePath/${activityName}.kt"))

    val fragment = cleanArchFragment(
        applicationPackage = applicationPackage,
        featurePackage = featurePackage,
        fragmentName = fragmentName,
        fragmentViewBindingName = fragmentViewBindingName,
        viewModelName = viewModelName
    )
    save(fragment, srcOut.resolve("$basePath/${activityName}.kt"))

    val viewModel = cleanArchViewModelSimple(
        featurePackage = featurePackage,
        viewModelName = viewModelName
    )
    save(viewModel, srcOut.resolve("$basePath/${viewModelName}.kt"))

}
